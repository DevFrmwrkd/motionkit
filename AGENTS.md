...
- **Hosting (secondary/dev)**: Cloudflare Workers (OpenNext) → `motionkit.frmwrkd-media.workers.dev`
...
- **Dev/Preview**: https://motionkit.frmwrkd-media.workers.dev (Cloudflare Workers via OpenNext)
- **Convex (canonical)**: `https://superb-oriole-955.convex.cloud` — must match Vercel prod env everywhere.
